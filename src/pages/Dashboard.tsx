      ...
      swapy.onSwapEnd((event) => {
        ...
        slots.forEach(slot => {
          ...
          if (slot.getAttribute('data-swapy-slot') === fromSlot) {
            slot.setAttribute('data-swapy-slot', `drop-place${slot.parentElement?.querySelector('h4')?.textContent}-${Date.now()}`);
        ...
        itemsElementsList.forEach(item => {
          if (item.getAttribute('data-swapy-item') === draggingItem) {
            item.parentElement?.setAttribute('data-swapy-slot', `${draggingItem}-${Date.now()}`);
          ...
          //update task status
          let task = tasks.find(task => task.id === taskItemElement.getAttribute('data-id'));
          if (task) {
            let listType = taskItemElement.parentElement?.parentElement?.parentElement?.querySelector('h4')?.textContent;
            if (listType){
              task.status = listType;
              updateTask(task);
              
              //update checkbox
              let taskCheckbox = taskItemElement.querySelector('input[type="checkbox"]') as HTMLInputElement;
              if (taskCheckbox) {
                taskCheckbox.checked = task.status === 'Done';
              }

              //update task status
              let taskStatus = taskItemElement.querySelector('.task-status p') as HTMLElement;
              if (taskStatus) {
        ...
        taskHolders.forEach(holder => {
          ...
          slot.className = 'slot';
          slot.setAttribute('data-swapy-slot', `drop-place${holder.parentElement?.querySelector('h4')?.textContent}-${Date.now()}`);
      ...
      {isSearching ? (
        ...
        selectedStatus === 'All tasks' ? (
          <>
            ...
            <TaskList listType="In progress" tasks={tasks.filter(task => task.status === 'In progress')} onDelete={onDelete} onUpdate={onUpdate} swapyInstance={swapyInstance} />
            <TaskList listType="Done" tasks={tasks.filter(task => task.status === 'Done')} onDelete={onDelete} onUpdate={onUpdate} swapyInstance={swapyInstance} />
          </>
        ) : (
          <TaskSearch searchTerm={searchTerm} tasks={tasks.filter(task => task.status === selectedStatus)} onDelete={onDelete} onUpdate={onUpdate} swapyInstance={swapyInstance}/>
        )
      )}
    </div>
  );
